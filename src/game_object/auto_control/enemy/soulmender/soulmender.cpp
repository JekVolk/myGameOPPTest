...
Soulmender::Soulmender(const Vector2 &position, const std::string &imagePath, int radius,
                       ...
                       int xp, int damage) : Enemy(position, imagePath, radius, scale, maxHp, speed, xp), _restoreHP(damage) {}

void Soulmender::heal(std::vector<std::unique_ptr<DamagerEnemy>> &enemies)
{
